<div align="center">

# Hey there, I'm Purush 👋

### Full-Stack Developer · React Enthusiast · 3D Web Explorer

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/purush34)
[![ORCID](https://img.shields.io/badge/ORCID-A6CE39?style=for-the-badge&logo=orcid&logoColor=white)](https://orcid.org/0009-0002-6136-9680)
[![GitHub followers](https://img.shields.io/github/followers/purush-o7?style=for-the-badge&logo=github&logoColor=white&color=181717)](https://github.com/purush-o7)

</div>

---

## 🚀 About Me

I'm a passionate full-stack developer who loves building **interactive, educational, and visually stunning web experiences**. I specialize in the React ecosystem and have a deep love for 3D web graphics, performance optimization, and developer tooling.

- 🔭 Currently building apps with **Next.js**, **React**, **Supabase**, and **Three.js / React Three Fiber**
- 🧠 Obsessed with making complex concepts **intuitive and interactive**
- 🎯 495+ contributions in the last year and counting
- 🌐 Exploring the intersection of **3D graphics** and **modern web development**
- 💡 I believe in learning by doing — every project I build teaches something new

---

## 🛠️ Tech Stack

**Frontend**

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)

**3D & Animation**

![Three.js](https://img.shields.io/badge/Three.js-000000?style=for-the-badge&logo=three.js&logoColor=white)
![React Three Fiber](https://img.shields.io/badge/React_Three_Fiber-000000?style=for-the-badge&logo=react&logoColor=61DAFB)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-0055FF?style=for-the-badge&logo=framer&logoColor=white)

**Backend & Database**

![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-005571?style=for-the-badge&logo=fastapi)

**Tools & Ecosystem**

![TanStack Query](https://img.shields.io/badge/TanStack_Query-FF4154?style=for-the-badge&logo=react-query&logoColor=white)
![Zod](https://img.shields.io/badge/Zod-3E67B1?style=for-the-badge&logo=zod&logoColor=white)
![shadcn/ui](https://img.shields.io/badge/shadcn%2Fui-000000?style=for-the-badge&logo=shadcnui&logoColor=white)

---

## 🌟 Featured Projects

### 🎣 [react-hooks-visualized](https://github.com/purush-o7/react-hooks-visualized)
> *An interactive, themed learning platform for React Hooks & TanStack Query*

Every hook is taught through a **real-world metaphor** — spy networks, space launches, DJ booths — so the concepts *stick*, not just the syntax. Features live interactive playgrounds where you can break things and see how hooks behave in real-time.

**Built with:** Next.js 16 · React 19 · TypeScript · Tailwind CSS v4 · TanStack Query v5 · Motion

---

### 🌌 [r3f-visualized](https://github.com/purush-o7/r3f-visualized)
> *An interactive learning platform for React Three Fiber — from Three.js fundamentals to production-ready 3D on the web*

Every concept has a **live, interactive 3D demo** you can play with directly in the browser. Covers everything from scene basics to custom shaders, physics with Rapier, post-processing effects, and production deployment.

**Built with:** Next.js 16 · React Three Fiber · Three.js · Drei · Rapier · Leva · Tailwind CSS v4

---

### 💪 [rep-tracker](https://github.com/purush-o7/rep-tracker)
> *A mobile-first gym tracker to log workouts, build streaks, track progress, and reach fitness goals*

Features a 120+ exercise library, routine management, streak tracking with a public leaderboard, a partner system, and progress charts. Backed by a robust Supabase PostgreSQL schema with Row Level Security on every table.

**Built with:** Next.js 16 · Supabase · PostgreSQL · shadcn/ui · Recharts · React Hook Form · Zod

---

### ⚡ [fastapi-starter-kit](https://github.com/purush-o7/fastapi-starter-kit)
> *A production-ready starter kit bridging FastAPI backends with Next.js frontends*

A clean foundation for spinning up full-stack projects fast — FastAPI handles backend logic while Next.js delivers a polished, performant frontend experience.

**Built with:** FastAPI · Next.js · TypeScript

---

## 📊 GitHub Stats

<div align="center">

[![Purush's GitHub stats](https://github-readme-stats.vercel.app/api?username=purush-o7&show_icons=true&theme=tokyonight&hide_border=true&count_private=true)](https://github.com/purush-o7)

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=purush-o7&layout=compact&theme=tokyonight&hide_border=true)](https://github.com/purush-o7)

[![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=purush-o7&theme=tokyonight&hide_border=true)](https://github.com/purush-o7)

</div>

---

## 💬 Let's Connect

I'm always up for a good chat about React internals, 3D graphics on the web, or building things that make people go "whoa". Feel free to reach out!

[![LinkedIn](https://img.shields.io/badge/Connect_on_LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/purush34)

---

<div align="center">

*"The best way to understand something is to build it, break it, and build it again."*

⭐ **If you find my projects useful, consider giving them a star!** ⭐

</div>
